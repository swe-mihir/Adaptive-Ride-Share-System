...
class DatabaseManager:
    ...
    def save_metrics_snapshot(self, run_id, sim_type, metrics, sim_time):
        """Save a metrics snapshot - matches MetricsTracker structure"""
        try:
            cursor = self.connection.cursor()

            # Extract data from your actual metrics structure
            cumulative = metrics.get('cumulative', {})
            carpooling = metrics.get('carpooling', {})
            ...
            # Map your metrics to database columns
            total_requests = safe_get(cumulative, 'total_requests')
            total_matches = safe_get(cumulative, 'total_matches')
            total_quits = safe_get(cumulative, 'total_quits')
            total_trips = safe_get(carpooling, 'total_trips')

            # Live state from simulator
            active_drivers = safe_get(live_state, 'active_drivers')
            active_requests = safe_get(live_state, 'active_requests')
            active_trips = safe_get(live_state, 'active_trips')
            ...
            cursor.execute(
                ...
                (
                    ...
                    active_requests,
                    active_trips,
                    0,  # total_passengers - calculate if needed
                    total_requests,
                    total_matches,
                    total_quits,
                    safe_get(cumulative, 'total_cost', 0.0),
                    safe_get(cumulative, 'avg_waiting_time', 0.0),
                    0.0,  # total_trip_time - not tracked in your metrics
                    safe_get(carpooling, 'avg_pool_size', 0.0)
                )
            )
            self.connection.commit()
            cursor.close()
